...
object Commons {
    ...
    val outputMediaFile: File?
        get() {
            val mediaStorageDir = File(
                ...
                Environment.DIRECTORY_PICTURES), "News")
    ...
    val outputMediaFileVideo: File?
        get() {
            val mediaStorageDir = File(
                Environment.getExternalStoragePublicDirectory(
                Environment.DIRECTORY_MOVIES), "News")
    ...
    /*fun seTypeFaceTextView(view: NewsTextView, path: Int) {
    ...
    fun loadImageGlide(url: String, view: ImageView) {
        Glide.with(App.getAppContext()).load(url).thumbnail(0.1f).apply(RequestOptions().placeholder(R.drawable.background_trans).error(R.drawable.background_trans)).diskCacheStrategy(
            DiskCacheStrategy.ALL).into(view)
    }

    fun loadImageGlideOverride(url: String, view: ImageView, size: Int) {
        Glide.with(App.getAppContext()).load(url).thumbnail(0.1f).apply(RequestOptions().placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).override(size)).diskCacheStrategy(
            DiskCacheStrategy.ALL).into(view)
    }

    fun loadImageGlideOverride(url: String, view: ImageView, with: Int, height: Int) {
        Glide.with(App.getAppContext()).load(url).thumbnail(0.1f).apply(RequestOptions().placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).override(with, height)).diskCacheStrategy(
            DiskCacheStrategy.ALL).into(view)
    }

    fun loadImageGlide(url: String, view: ImageView, placeHolder: Int, error: Int) {
        Glide.with(App.getAppContext()).load(url).thumbnail(0.1f).apply(RequestOptions().placeholder(placeHolder).error(error)).diskCacheStrategy(
            DiskCacheStrategy.ALL).into(view)
    }

    fun loadImageGlideOverride(url: String, view: ImageView, placeHolder: Int, error: Int, size: Int) {
        Glide.with(App.getAppContext()).load(url).thumbnail(0.1f).apply(RequestOptions().placeholder(placeHolder).error(error).override(size)).diskCacheStrategy(
            DiskCacheStrategy.ALL).into(view)
    }

    fun loadImageGlideOverride(url: String, view: ImageView, placeHolder: Int, error: Int, with: Int, height: Int) {
        Glide.with(App.getAppContext()).load(url).thumbnail(0.1f).apply(RequestOptions().placeholder(placeHolder).error(error).override(with, height)).diskCacheStrategy(
            DiskCacheStrategy.ALL).into(view)
    }
